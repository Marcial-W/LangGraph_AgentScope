# -*- coding: utf-8 -*-
"""
内容生成智能体 (Content Agent)
负责生成文案、图文、视频、互动帖等多模态内容
"""

from agentscope.agent import AgentBase
from agentscope.message import Msg
from typing import Dict, List, Any
import json
import random


class ContentAgent(AgentBase):
    """内容生成智能体 - 负责内容制作组的角色"""
    
    def __init__(self, name: str = "content_agent"):
        super().__init__()
        self.name = name
        self.content_templates = {
            "微博": {
                "字数限制": 140,
                "特点": "简洁有力，话题标签"
            },
            "小红书": {
                "字数限制": 1000,
                "特点": "图文并茂，生活化"
            },
            "抖音": {
                "时长限制": "15-60秒",
                "特点": "视觉冲击，音乐节奏"
            }
        }
        
    async def reply(self, message: Msg) -> Msg:
        """处理内容生成请求"""
        content = message.content if isinstance(message.content, str) else str(message.content)
        
        # 解析内容需求
        if "文案" in content:
            response = self._generate_copywriting(content)
        elif "图文" in content:
            response = self._generate_image_text(content)
        elif "视频" in content:
            response = self._generate_video_script(content)
        elif "互动" in content:
            response = self._generate_interactive_post(content)
        else:
            response = self._generate_multimodal_content(content)
            
        return Msg(
            role="assistant",
            name=self.name,
            content=response
        )
    
    def _generate_copywriting(self, request: str) -> str:
        """生成营销文案"""
        platform = self._detect_platform(request)
        
        copywriting_examples = {
            "微博": [
                "🔥 当AI遇上生活，每一天都是智能新体验！#AI生活 #智能科技 #未来已来",
                "💡 告别繁琐，让AI成为你的生活助手～今天你AI了吗？#智能生活 #科技改变生活"
            ],
            "小红书": [
                "姐妹们！发现了一个超智能的生活神器🌟\n\n✨ 智能识别需求\n✨ 个性化推荐方案\n✨ 24小时贴心服务\n\n用了一周，感觉生活品质都提升了！真心推荐给追求品质生活的你们～\n\n#智能生活 #品质生活 #科技好物"
            ],
            "抖音": [
                "【15秒短视频文案】\n开场：还在为XX烦恼吗？\n展示：一键解决，就是这么简单！\n结尾：Get同款，评论区链接🔗"
            ]
        }
        
        platform_key = platform if platform in copywriting_examples else "微博"
        selected = random.choice(copywriting_examples[platform_key])
        
        return f"✍️ {platform}文案生成：\n\n{selected}\n\n[提示：已根据平台特性优化字数和风格]"
    
    def _generate_image_text(self, request: str) -> str:
        """生成图文内容"""
        image_text = {
            "标题": "用AI开启智能生活新纪元",
            "图片建议": [
                "图1：产品使用场景图（温馨家居环境）",
                "图2：功能特性对比图（直观展示优势）",
                "图3：用户好评截图（增强信任度）"
            ],
            "正文": """
在这个快节奏的时代，我们都在寻找让生活更轻松的方式。

今天想和大家分享我的智能生活新发现——

🏠 智能家居控制
只需一句话，灯光、温度、音乐随心调节

📱 个性化推荐
基于你的喜好，推荐最适合的生活方案

⏰ 智能时间管理
合理规划每一天，工作生活两不误

最打动我的是它真的懂你想要什么，不是冷冰冰的机器，更像是贴心的生活管家。

你们有什么智能好物推荐吗？评论区交流呀～
""",
            "配图说明": "建议配3-6张高质量实拍图，展示产品细节和使用场景"
        }
        
        return f"🖼️ 图文内容方案：\n{json.dumps(image_text, ensure_ascii=False, indent=2)}"
    
    def _generate_video_script(self, request: str) -> str:
        """生成视频脚本"""
        video_script = {
            "时长": "30秒",
            "场景列表": [
                {
                    "时间": "0-5秒",
                    "画面": "问题场景：用户面对繁琐的日常任务",
                    "文案": "每天都在重复这些琐事？",
                    "音效": "轻快背景音乐"
                },
                {
                    "时间": "5-20秒",
                    "画面": "解决方案展示：AI产品使用演示",
                    "文案": "有了它，一切变得如此简单",
                    "音效": "节奏感增强"
                },
                {
                    "时间": "20-30秒",
                    "画面": "效果展示：用户轻松愉快的生活状态",
                    "文案": "智能生活，从现在开始",
                    "音效": "温馨结尾音"
                }
            ],
            "拍摄要点": "注重画面转场流畅性，突出产品易用性"
        }
        
        return f"🎬 视频脚本：\n{json.dumps(video_script, ensure_ascii=False, indent=2)}"
    
    def _generate_interactive_post(self, request: str) -> str:
        """生成互动帖内容"""
        interactive_post = {
            "类型": "投票互动",
            "主题": "你最期待的AI功能是？",
            "选项": [
                "A. 智能语音助手",
                "B. 个性化推荐",
                "C. 自动化任务处理",
                "D. 情感陪伴"
            ],
            "引导语": "快来投票，评论区说说你的想法～抽3位送智能好礼🎁",
            "预期效果": "提高用户参与度，收集用户需求"
        }
        
        return f"💬 互动帖方案：\n{json.dumps(interactive_post, ensure_ascii=False, indent=2)}"
    
    def _generate_multimodal_content(self, request: str) -> str:
        """生成多模态综合内容"""
        content_package = {
            "内容组合": "图文+短视频+互动话题",
            "发布策略": "分时段、多平台联动发布",
            "核心卖点": "AI让生活更美好",
            "风格调性": "科技感+温度感并存"
        }
        
        return f"🎨 多模态内容包：\n{json.dumps(content_package, ensure_ascii=False, indent=2)}"
    
    def _detect_platform(self, content: str) -> str:
        """检测目标平台"""
        platforms = ["微博", "小红书", "抖音"]
        for platform in platforms:
            if platform in content:
                return platform
        return "微博"  # 默认平台
